...
@synthesize lastPinLocation = _lastPinLocation;
...
- (void)viewDidLoad
{
    ...
    int currentTripID = [userDefaults integerForKey:@"currentTripID"];
    if (self.isContinue) {
    }
    else
    {
        currentTripID++;
        [userDefaults setInteger:0 forKey:@"currentPinID"];
    }
    
    NSLog(@"%d", currentTripID);
    
    self.path = [NSString stringWithFormat:@"%@/trip%d",[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)objectAtIndex:0], currentTripID];
    NSLog(@"%@", self.path);
    ...
    if (self.isContinue) {
        self.infoDic = [[NSMutableDictionary alloc]initWithContentsOfFile:[NSString stringWithFormat:@"%@/infoDic.plist", self.path]];
    }
    else
    {
        self.infoDic = [[NSMutableDictionary alloc]initWithCapacity:0];
        [self.infoDic setValue:[NSString stringWithFormat:@"trip%d", currentTripID] forKey:@"tripName"];
        NSMutableArray *pinArray = [[NSMutableArray alloc]initWithCapacity:0];
        [self.infoDic setValue:pinArray forKey:@"pinArray"];
        [pinArray release];
    }
    ...
    audioFuncButton = [UIButton buttonWithType:UIButtonTypeCustom];
    audioFuncButton.frame = CGRectMake(270, 0, 50, 50);
    [self.view addSubview:audioFuncButton];
    audioFuncButton.backgroundColor = [UIColor lightGrayColor];
    
    videoFuncButton = [UIButton buttonWithType:UIButtonTypeCustom];
    videoFuncButton.frame = CGRectMake(270, 0, 50, 50);
    [self.view addSubview:videoFuncButton];
    videoFuncButton.backgroundColor = [UIColor lightGrayColor];
    [videoFuncButton addTarget:self action:@selector(takeVideo:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)rightUnfold
{
    if (rightShow == NO) {
        ...
        audioFuncButton.frame = CGRectMake(270, 150, 50, 50);
        videoFuncButton.frame = CGRectMake(270, 200, 50, 50);
        [UIView commitAnimations];
    }
    else
    {
...
-(void)gotoBrowse:(UIButton *)sender
{
    ...
    browse.path = self.path;
    browse.annotation = self.currentAnnotation;
...
{
    NSLog(@"%@", [info valueForKey:UIImagePickerControllerMediaType]);
    if ([[info valueForKey:UIImagePickerControllerMediaType]isEqualToString:@"public.image"]) {
        UIImage *image = [info valueForKey:UIImagePickerControllerEditedImage];
        NSData *imageData = UIImagePNGRepresentation(image);
        NSString *imagePath = [NSString stringWithFormat:@"%@/image%d_%d.png", self.path, self.currentAnnotation.pinID, self.currentAnnotation.currentPicID];
        self.currentAnnotation.currentPicID++;
        
        //保存
        NSMutableArray *pinArray = [self.infoDic valueForKey:@"pinArray"];
        Annotation *annotation = [pinArray lastObject];
        NSMutableDictionary *dic = [[NSMutableDictionary alloc]initWithObjectsAndKeys:imagePath, @"path", [NSDate date], @"date", nil];
        [annotation.pictureArray addObject:dic];
        [dic release];
    ...
    else if ([[info valueForKey:UIImagePickerControllerMediaType]isEqualToString:@"public.movie"])
    {
        ...
        NSString *imagePath = [NSString stringWithFormat:@"%@/image%d_%d.mov", self.path, self.currentAnnotation.pinID, self.currentAnnotation.currentPicID];
        self.currentAnnotation.currentPicID++;
